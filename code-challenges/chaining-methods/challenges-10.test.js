...
const count = (target, input) => input.reduce((total, arr) => total + arr.reduce((total, num) => num === target ? total + 1 : total, 0), 0);
...
const totalSum = (input) => input.reduce((total, arr) => total + arr.reduce((total, num) => total + num, 0), 0);
...
const divisibleByFiveTwoToThePower = (input) => input.map(arr => arr.filter(value => typeof value === 'number' && value % 5 === 0).map(value => Math.pow(2, value)));
...
let findMaleAndFemale = (data) => data.filter(character => character.gender === 'male' || character.gender === 'female').reduce((joined, current) => !joined ? current.name : `${joined} and ${current.name}`, '');
