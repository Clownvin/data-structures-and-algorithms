...
const charCode = (arr) => arr.map(character => character.charCodeAt(0));
...
const evenOdd = (arr) => arr.map(num => isNaN(num) ? 'N/A' : num % 2 ? 'odd' : 'even');
...
const extractAbilities = (arr) => arr.map(abilityInfo => abilityInfo.ability.name);
...
const extractStats = (arr) => arr.map(statInfo => ({
