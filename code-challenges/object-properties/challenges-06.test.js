...
const totalCharacters = (arr) => {
  ...
  for (const family of arr) {
    if (!characters.includes(family.name)) {
      characters.push(family.name);
    }
    if (!characters.includes(family.spouse)) {
      characters.push(family.spouse);
    }
...
const houseSize = (arr) => {
  const sizes = [];
  for (const family of arr) {
    let obj = { house: family.house, members: 1 + family.children.length };
    if (family.spouse) {
      obj.members++;
    }
    sizes.push(obj);
...
const houseSurvivors = (arr) => {
  const survivors = [];
  for (const family of arr) {
    let obj = { house: family.house, members: family.children.length };
    if (!deceasedSpouses.includes(family.name)) {
      obj.members++;
    }
    if (family.spouse && !deceasedSpouses.includes(family.spouse)) {
      obj.members++;
    }
    survivors.push(obj);
  }
