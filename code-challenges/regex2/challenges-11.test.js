...
const validateEmail = (email) => email.match(/^[\d\w]+(.[\d\w]+)?@[\d\w]+.((com)|(org)|(net)){1}$/);
...
const validatePhoneNumber = (phoneNumber) => phoneNumber.match(/^((\(\d{3}\))|(\d{3}))(\s|-)?\d{3}(\s|-)?\d{4}$/);
...
const findTagNames = elements => elements.reduce((closingTags, current) => closingTags.concat(current.match(/<\/[\w\d]+>/g)), []).map((tag) => tag.slice(1, tag.length - 1));
